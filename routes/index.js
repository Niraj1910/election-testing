...
router.get("/alliances", async (req, res) => {
  const alliances = await AllianceModel.find()
    ...
    .populate("parties");
  res.render("alliance.ejs", { alliances });
});
...
router.get("/edit-alliance/:id", async (req, res) => {
  ...
  res.render("edit-alliance", { alliance, allParties });
});

router.get("/create-alliance", async (req, res) => {
  const parties = await Party.find({}, "_id party");
  res.render("create-alliance.ejs", { parties });
});

//Temporary create election route
router.get("/temp-create-election", async (req, res) => {
  const parties = await Party.find({}, "_id party");
...
router.get("/temp-election-list", async (req, res) => {
  try {
    const elections = await TempElection.find({})
      .populate("electionInfo.partyIds") // Populate party information
      .populate({
        path: "electionInfo.candidates",
