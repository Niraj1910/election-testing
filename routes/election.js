...
router.patch("/temp-election/candidate/add", async (req, res) => {
	try {
		...
		const newCandidates = candidates.map(
			(candidateId) => new CandidateElectioModel({ election, candidate: candidateId }),
		);

		const newAddedCandidates = await CandidateElectioModel.bulkSave(newCandidates);
	...
	async (req, res) => {
		try {
			...
			const deletedPartyCandidate =
				await CandidateElectioModel.findOneAndDelete({
					election: electionId,
